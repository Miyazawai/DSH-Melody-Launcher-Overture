...
- **OfficeCLI 工具自动补装（镜像优先）**：启动含 officecli 技能的整合包时，启动器自动准备好这个 33MB 外部二进制——解析 GitHub Release 后按候选源链（用户 GitHub 镜像 → 直连 → 内置公共镜像）逐个下载、SHA256/大小校验、装进机器级托管目录（`userData/dsh-tools/`，跨包共享、不进快照）并注入 DSH 子进程 PATH；技能自带的 install.ps1 联网自愈从此不会被触发。24 小时版本缓存免重复解析；准备等待设 3 分钟硬顶，超时/失败一律不阻塞启动（后台续传，下次启动命中缓存）。整合包是否需要由 `skills/officecli*` 目录名秒判，不相关包零开销。每个候选源带断流看门狗（15 秒无字节掐掉换源，专治大陆直连"握手成功永不回数据"的黑洞）与 5 分钟单源硬顶；下载进度实时显示在首页启动按钮小字（「正在准备 Office 工具：x%…」），整合包页横幅同步。
