...
- **整合包页重做**：行显示版本徽章、「自动」标记、DSH 版本与插件/技能/预设计数；支持内联重命名、新建、导出（不含会话与登录）、删除前显示磁盘占用并说明连环境数据一并删除；激活包也可删除（删后自动落到剩余整合包）；「停用当前整合包」取消。
- **零包引导态**：删光所有整合包后不再自动新建兜底包——整合包页改为「新建 → 下载 DSH 版本 → 装插件/技能」三步引导卡，首页按钮转为「新建整合包」入口；零包状态下新建包或下载版本会自动成为当前包（引导闭环）；无包时启动 DSH 会被拦下并引导去整合包页。
